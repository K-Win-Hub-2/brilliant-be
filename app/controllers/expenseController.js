...
exports.expenseFilter = async (req, res) => {
  ...
  try {
    ...
    const bankResult = await Expense.find(query)
      .populate("relatedBankAccount relatedAccounting")
    ...
    const cashResult = await Expense.find(query2)
      .populate("relatedAccounting relatedCashAccount")
      .populate("createdBy", "givenName");
    console.log(cashResult, "cashRes");
    ...
    const BankTotal = bankResult.reduce((total, sale) => {
      let current = currencyList.filter(
        (currency) => currency.name === sale.finalCurrency
      )[0]?.exchangeRate;
...
exports.filterExpense = async (req, res, next) => {
  try {
    // const checkId = await Expense.find(id);
    // console.log(checkId, "check");
    let query = { relatedAccounting: { $exists: true }, isDeleted: false };
