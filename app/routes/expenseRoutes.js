...
module.exports = (app) => {
  ...
  app
    .route("/api/expenses")
    .get(verifyToken, catchError(expense.listAllExpenses));
  app
    .route("/api/expenses/get-date")
    .get(verifyToken, catchError(expense.getwithExactDate));
  app.route("/api/expenses/filter").get(catchError(expense.expenseFilter));
  app.route("/api/expenses-filter").get(catchError(expense.filterExpense));
