...
exports.getItemAndShareHolders = async (datas) => {
    try{
       ...
       let result = await itemAndShareHolder.find(query).limit(paginationHelpers.limit).skip(paginationHelpers.skip).sort(sortByAscending).exec()
...
exports.getItemAndShareHolderById = async (id) => {
    try{
       let result = await itemAndShareHolder.findById(id)
        return result; 
    }catch(err){
        console.log("Error is", err.message)
    }
...
exports.updateItemAndShareHolder = async (id, datas) => {
    try{
       let result = await itemAndShareHolder.findByIdAndUpdate(id, datas, { new: true })
       return result; 
    }catch(err){
        console.log("Error is", err.message)
    }
...
exports.deleteItemAndShareHolder = async (id) => {
    try{
       let result = await itemAndShareHolder.findByIdAndUpdate(id, {isDeleted: true}, { new: true })
       return result; 
    }catch(err){
        console.log("Error is", err.message)
    }
