...
const { createItemAndShareHolder } = require("./itemAndShareHolderService");
...
exports.getAllItems = async (datas) => {
  try {
    ...
    let result = await items
      ...
      .populate({
        path: "relatedItemTitle",

        populate: {
          path: "relatedSubCategory",
          model: "SubCategories",
        },
      })
      .populate({
...
exports.createItems = async (datas) => {
  try {
    ...
    //if there are too many shareholder then change in this like loop through the shareholder array
    const itemShareHolderResult = await createItemAndShareHolder({
      relatedShareHolder: datas.relatedShareHolder.shareholder_id,
      percent: datas.relatedShareHolder.percent,
      relatedItem: result._id,
    });
    return result;
  } catch (err) {
    console.log("Error is", err.message);
...
exports.deleteItem = async (id) => {
  try {
    let result = await items.findByIdAndUpdate(
      id,
      { isDeleted: true },
      { new: true }
    );
