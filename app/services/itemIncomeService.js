...
exports.getItemIncome = async (datas) => {
    try{
       ...
       let result = await itemIncomeTotal.find(query).limit(paginationHelpers.limit).skip(paginationHelpers.skip).sort(sortByAscending).exec()
...
exports.createItemIncome = async (datas) => {
    try{
       ...
       if(searchIncome){
        result = await itemIncomeTotal.findByIdAndUpdate(searchIncome._id, { $inc: { quantity: datas.quantity, amount: datas.amount, purchasePrice: datas.purchasePrice}}, { new: true})
       }else{
...
exports.updateitemIncomeTotal = async (id, datas) => {
    try{
       let result = await itemIncomeTotal.findByIdAndUpdate(id, datas, { new: true })
       return result; 
    }catch(err){
        console.log("Error is", err.message)
    }
...
exports.deleteitemIncomeTotal = async (id) => {
    try{
       let result = await itemIncomeTotal.findByIdAndUpdate(id, {isDeleted: true}, { new: true })
       return result; 
    }catch(err){
        console.log("Error is", err.message)
    }
